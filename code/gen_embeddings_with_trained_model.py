...
from NewLazyLoadDatasetV4 import convert_dtype_dataframe
...
train_dataset, valid_dataset, test_dataset = prepare_datasets(data_dir=data_dir, project_list=project_list,
                                                              bert_model=bert_model, bert_tokenizer=bert_tokenizer,
                                                              device=device)

train_text_embeddings, valid_text_embeddings, test_text_embeddings = prepare_text_embeddings(train_dataset,
                                                                                             valid_dataset,
                                                                                             test_dataset,
...
def line_evaluation(runs_dir):
    ...
    def deal_with_attns(item, text_attns, pred, commit2codes, idx2label, graph_attns=None, line_code_label=None,
                        only_adds=True, have_graph_attns=True, have_text_attns=True):
        if not have_text_attns and not have_text_attns:
            return [], [], []
        metrics_line, metrics_node_1, metrics_final = [], [], []
        ...
        if have_graph_attns:
            ...
            # threshold_index = int(len(node_attn_df) * 1)  # Top 20%
            ...
            for i, row in node_attn_df.iterrows():
            ...
            tokenized_node_attn_df = DataFrame()
        ...
        if have_graph_attns and have_text_attns:
            final_scores = pd.merge(node_scores,line_scores, how='left', on='idx', suffixes=('', '_combined'))
            scaler = MinMaxScaler()
            final_scores['score'] = final_scores['score'].fillna(0)
            # Scale the 'score' and 'node_score' columns
            final_scores[['node_score']] = scaler.fit_transform(final_scores[['node_score']])
            scaler = MinMaxScaler()
            final_scores[['score']] = scaler.fit_transform(final_scores[['score']])
            # Fit the scaler on 'node_score' to get the min and max values
            node_score_min = final_scores['node_score'].min()
            node_score_max = final_scores['node_score'].max()
            # # print(f"node_score_min {node_score_min}, node_score_max {node_score_max}", flush=True)
            
            # # Apply min-max scaling to 'score' using the min and max of 'node_score'
            if node_score_max - node_score_min > 1e-8 :
                final_scores['score'] = (final_scores['score'] - node_score_min) / (node_score_max - node_score_min)
            
            # Now you can add them together to get the final score
            final_scores['final_score'] = final_scores['score'] + final_scores['node_score']
            # final_scores['final_score'] = np.sqrt(final_scores['score'] * final_scores['node_score'])
            # final_scores['final_score'] = final_scores[['score', 'node_score']].max(axis=1)  # Max of the two columns
            # final_scores['score_log'] = np.log(final_scores['score'] + 1e-5)
            # final_scores['node_score_log'] = np.log(final_scores['node_score'] + 1e-5)
            
            # # Combine the transformed scores
            # final_scores['final_score'] = final_scores['score_log'] + final_scores['node_score_log']
            # final_scores['final_score'] = final_scores[['score', 'node_score']].median(axis=1)  # Mean of the two columns

            # def custom_aggregation(row):
            #     if row['score'] > 0.5:
            #         return row['score'] * 0.6 + row['node_score'] * 0.4
            #     else:
            #         return row['node_score']
            # print(final_scores[['score', 'node_score']])
            # final_scores['final_score'] = final_scores.apply(custom_aggregation, axis=1)
        ...
        metrics_line = get_line_level_metrics(line_scores['score'].tolist(), line_scores['label'].tolist()) if have_text_attns else []
        metrics_node_1 = get_line_level_metrics(node_scores['node_score'].tolist(), node_scores['label'].tolist()) if have_graph_attns else []
        # metrics_node = get_line_level_metrics(line_code_label_df['score'].tolist(), line_code_label_df['label'].tolist())
        metrics_final = get_line_level_metrics(final_scores['final_score'].tolist(), final_scores['label'].tolist()) if have_text_attns and have_graph_attns else []
    ...
    def evaluate(model, dataset, text_embeddings, line_eval=False, epoch=None, early_stopping=None):
        ...
        model.eval()
        # graph_model_state_dict = torch.load('/workspace/s2156631-thesis/results/20241118-092402v4_graph_only/1/model.bin', weights_only=False)
        # graph_model = HomogeneousGraphSequentialClassifierV2(in_channels=meta['in_channels'], out_channels=meta['out_channels'],
        #                                                device=device, with_manual_features=False,
        #                                                with_text_features=False, with_graph_features=True, manual_size=14).to(device)
        
        # graph_model.load_state_dict(graph_model_state_dict['model_state_dict'])
        data_loader = DataLoader(dataset, batch_size=batch_size, shuffle=False)
        graph_edges_tr = '_'.join(dataset.graph_edges) if dataset.graph_edges is not None else ''
        total_loss = 0
        all_prods = []
        all_labels = []
        all_embeddings = []
        ...
        with torch.no_grad():
            for batch in data_loader:
                ...
                # _,_, graph_attn_weights_1 = graph_model(data)
                all_graph_attns.extend(graph_attn_weights)
                # all_graph_attns_1.extend(graph_attn_weights_1)
        ...
        col_names = ['commit_id', 'prod', 'pred', 'label']
        jitfine_pred = pd.read_csv('/workspace/ThesisDatasetGraph/new_effort/jitfine_predictions.csv', names=col_names, sep='\t')
        print(jitfine_pred)
        ...
        for commit_project_id, example in df.iterrows():
            ...
            if len(all_graph_attns) > 0:
                graph_attn = all_graph_attns[commit_idx]
                id_mapping_path = os.path.join(data_dir, project_name, commit_id + graph_edges_tr + '_id_mapping.pt')
                ...
                for node_attn_idx in range(0, len(graph_attn)):
                    try:
                        ...
                        raise e
                graph_attns = (graph_attn, node_texes)
            else:
                graph_attns = None
            ...
            # calculate
            commit_row = jitfine_pred[jitfine_pred['commit_id'] == commit_id]
            # print(commit_row)
            if int(label) == 1 and int(pred) == 1 and '[ADD]' in input_tokens and (not commit_row.empty) and commit_row['pred'].iloc[0] == True:
            # if int(label) == 1 and int(pred) == 1 and '[ADD]' in input_tokens:
                ...
                line_metric, graph_node_metric, combined_metric = deal_with_attns(
                    example, attn,
                    pred, cur_codes,
                    cur_labels, graph_attns=graph_attns,  line_code_label=line_code_label, only_adds=True)
                line_metrics.append(line_metric)
                node_metrics_1.append(graph_node_metric)
                combined_metrics.append(combined_metric)
            commit_idx += 1

        metric_types = [line_metrics, node_metrics_1, combined_metrics]
        metric_values = []
        for metrics in metric_types:
            ...
            metric_values.append((round(np.mean(top_10_acc), 4), round(np.mean(top_5_acc), 4),
        ...
        return metric_values, RF_result

    def calculate_metrics(prods, labels):
        prods = np.array(prods).flatten()
        preds = [1 if p > 0.3 else 0 for p in prods]
    ...
    def eval_metrics(result_df):
        ...
        return prec, f1, AUC, recall_20_percent_effort, effort_at_20_percent_LOC_recall, p_opt
    ...
    def eval_result(result_path, features_path):
        ...
        prec, f1, auc, recall_20_percent_effort, effort_at_20_percent_LOC_recall, p_opt = eval_metrics(
            RF_result)
        print(
            'Prec: {:.4f}, F1: {:.4f}, AUC: {:.4f}, PCI@20%LOC: {:.4f}, Effort@20%Recall: {:.4f}, POpt: {:.4f}'.format(
                prec, f1, auc, recall_20_percent_effort, effort_at_20_percent_LOC_recall, p_opt))
        return (prec, f1, auc, recall_20_percent_effort, effort_at_20_percent_LOC_recall, p_opt)

    _, train_results = evaluate(model, train_dataset, train_text_embeddings, line_eval=False)
    _, valid_results = evaluate(model, valid_dataset, valid_text_embeddings, line_eval=False)
    line_results, test_results = evaluate(model, test_dataset, test_text_embeddings, line_eval=True)
    all_results = pd.concat([train_results, valid_results, test_results])
    all_results.to_csv(runs_dir + '/' + "all_predictions.csv", sep='\t', index=None,
                       header=['commit_id', 'prod', 'pred', 'label'])
    ...
    return [commit_results + line_res for line_res in line_results]

val_results = []
val_results_1 = []
val_results_2 = []
val_results_3 = []
exp_path = '/workspace/s2156631-thesis/results/final_results/20241117-022626v4_no_ast_128_no_man/'
for i in range(0, 10):
    runs_dir = exp_path + str(i)
    results = line_evaluation(runs_dir)
    val_results_1.append(results[0])
    val_results_2.append(results[1])
    val_results_3.append(results[2])

val_results = [val_results_1, val_results_2, val_results_3]
for i in range(0, 3):
    val_means = np.mean(val_results[i], axis=0)
    val_stds = np.std(val_results[i], axis=0)
    ...
    # Display results
    metrics = ['prec', 'f1', 'AUC', 'recall_20_percent_effort', 'effort_at_20_percent_LOC_recall',
               'p_opt', 'top_10_acc_line', 'top_5_acc_line', 'top_20_percent_LOC_recall_line',
               'effort_at_20_percent_LOC_recall_line', 'IFA_line']
    ...
    output_file = exp_path + '/results_' + str(i) + '.csv'
...
end_time_exp = datetime.now().strftime("%Y%m%d-%H%M%S")
print(f"start time for evaluations {time_exp}", flush=True)
print(f"end time for evaluations {end_time_exp}", flush=True)